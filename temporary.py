...
class LLMNode:
    ...
    def _initialize_google_services(self):
        """
        Initialize Google services (Calendar and Gmail) with user authentication.
        
        This function guides the user through Google sign-in process, using OAuth2 authentication
        to securely access Google services without the user needing to provide API keys.
        
        The function:
        1. Attempts to load saved credentials for the user
        2. If no valid credentials exist, starts an OAuth flow to guide the user through sign-in
        3. Saves the credentials for future use
        4. Initializes and tests Google Calendar and Gmail services
        
        Returns:
            dict: A dictionary with service objects for 'calendar' and 'gmail'.
                  If initialization fails, the corresponding service will be None.
        """
        ...
        # Check if developer credentials are available
        if not GOOGLE_CLIENT_ID or not GOOGLE_CLIENT_SECRET:
            print(f"[{self.node_id}] Developer credentials unavailable. Contact application provider.")
            log_error("Google API client credentials missing. The application developer needs to set these up.")
            return services
        
        token_file_path = get_token_file_path(self.node_id)
        creds = None
        
        # Try to load existing credentials for this user
        if os.path.exists(token_file_path):
            try:
                with open(token_file_path, 'rb') as token:
                    creds = pickle.load(token)
                print(f"[{self.node_id}] Found saved Google sign-in information")
            except Exception as e:
                print(f"[{self.node_id}] Cannot load saved Google sign-in data: {str(e)}")
        ...
        try:
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    try:
                        print(f"[{self.node_id}] Refreshing Google sign-in")
                        creds.refresh(Request())
                    except Exception as e:
                        print(f"[{self.node_id}] Error refreshing Google sign-in: {str(e)}")
                        creds = None
                        # Delete invalid token file
                        if os.path.exists(token_file_path):
                            os.remove(token_file_path)
                
                # If no valid credentials, start a new sign-in flow
                if not creds:
                    print(f"[{self.node_id}] Starting Google sign-in process...")
                    ...
                    try:
                        ...
                        print(f"[{self.node_id}] Please sign in with your Google account in the browser window that opens")
                        print(f"[{self.node_id}] This allows the application to access your calendar and email")
                        
                        # Generate the authorization URL and open it in a web browser
                        auth_url, _ = flow.authorization_url(prompt='consent')
                        print(f"[{self.node_id}] Opening Google sign-in page in your browser...")
                        webbrowser.open(auth_url)
                        
                        # Run local server to receive the OAuth callback
                        print(f"[{self.node_id}] Waiting for you to complete Google sign-in...")
                        creds = flow.run_local_server(port=8080)
                        print(f"[{self.node_id}] Google sign-in successful!")
                    except Exception as e:
                        error_msg = f"Google sign-in error: {str(e)}"
                ...
                try:
                    ...
                    print(f"[{self.node_id}] Google sign-in information saved for future use")
                except Exception as e:
                    print(f"[{self.node_id}] Error saving Google sign-in information: {str(e)}")
            
            # Initialize Calendar service
            try:
                print(f"[{self.node_id}] Setting up Google Calendar...")
                calendar_service = build('calendar', 'v3', credentials=creds)
                
                # Test the connection
                calendar_list = calendar_service.calendarList().list().execute()
                print(f"[{self.node_id}] Google Calendar connected! Found {len(calendar_list.get('items', []))} calendars")
            ...
            try:
                print(f"[{self.node_id}] Setting up Gmail...")
                gmail_service = build('gmail', 'v1', credentials=creds)
                
                # Test the connection
                profile = gmail_service.users().getProfile(userId='me').execute()
                print(f"[{self.node_id}] Gmail connected! Using account: {profile.get('emailAddress')}")
            ...
            error_msg = f"Error during Google service initialization: {str(e)}"
            print(f"[{self.node_id}] {error_msg}")
            log_error(error_msg)
            return services
