...
interface AgentCandidateSelectorProps {
  ...
  onCandidateAccept: (candidateName: string, projectId?: string) => void;
  onCandidateReject: (candidateName: string, projectId?: string) => void;
}
...
const AgentCandidateSelector: React.FC<AgentCandidateSelectorProps> = ({ 
  ...
  if (!candidates || candidates.length === 0) {
    return <p className="text-sm text-gray-500">No candidates to display.</p>;
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {candidates.map((candidate, index) => (
          <AgentCandidateCard 
            ...
            candidate={candidate} 
            onAccept={(name) => onCandidateAccept(name, projectId)}
            onReject={(name) => onCandidateReject(name, projectId)}
          />
        ))}
      </div>
    </div>
